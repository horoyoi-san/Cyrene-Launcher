...
export default function hkrpgPage() {
  ...
  useEffect(() => {
    const fetchGame = async () => {
      try {
        ...
        const hkrpg = list.find((g: any) => g.biz === "hkrpg_global");

        if (!hkrpg) {
          toast.error("StarRail not found");
          return;
        }

        const parsed: Game = {
          name: hkrpg.display.name,
          biz: hkrpg.biz,
          background: hkrpg.display.background.url,
        };
...
const downloadGame = async () => {
  try {
    ...
    // 🔥 หา game จาก config
    const gameConfig = config.games.find(
      (g: any) => g.id === "StarRail" // หรือ map จาก biz ก็ได้
    );
