...
export default function hk4ePage() {
  ...
  useEffect(() => {
    const fetchGame = async () => {
      try {
        ...
        const hk4e = list.find((g: any) => g.biz === "hk4e_global");

        if (!hk4e) {
          toast.error("Genshin not found");
          return;
        }

        const parsed: Game = {
          name: hk4e.display.name,
          biz: hk4e.biz,
          background: hk4e.display.background.url,
        };
...
const downloadGame = async () => {
  try {
    ...
    // 🔥 หา game จาก config
    const gameConfig = config.games.find(
      (g: any) => g.id === "GenshinImpact" // หรือ map จาก biz ก็ได้
    );
