...
export default function hygPage() {
  ...
  useEffect(() => {
    const fetchGame = async () => {
      try {
        ...
        const hyg = list.find((g: any) => g.biz === "hyg_global");

        if (!hyg) {
          toast.error("PetitPlanet not found");
          return;
        }

        const parsed: Game = {
          name: hyg.display.name,
          biz: hyg.biz,
          background: hyg.display.background.url,
        };
...
const downloadGame = async () => {
  try {
    ...
    // 🔥 หา game จาก config
    const gameConfig = config.games.find(
      (g: any) => g.id === "PetitPlanet" // หรือ map จาก biz ก็ได้
    );
