...
export default function bh3Page() {
  ...
  useEffect(() => {
    const fetchGame = async () => {
      try {
        ...
        const bh3 = list.find((g: any) => g.biz === "bh3_global");

        if (!bh3) {
          toast.error("Impact3rd not found");
          return;
        }

        const parsed: Game = {
          name: bh3.display.name,
          biz: bh3.biz,
          background: bh3.display.background.url,
        };
...
const downloadGame = async () => {
  try {
    ...
    // 🔥 หา game จาก config
    const gameConfig = config.games.find(
      (g: any) => g.id === "Impact3rd" // หรือ map จาก biz ก็ได้
    );
